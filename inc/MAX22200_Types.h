...
struct ChannelConfig {
    ...
    ChannelConfig() 
        : enabled(false)
        , drive_mode(DriveMode::CDR)
        , bridge_mode(BridgeMode::HALF_BRIDGE)
        , parallel_mode(false)
...
struct FaultStatus {
    ...
    FaultStatus()
        ...
        , open_load(false)
        , plunger_movement(false)
        , undervoltage_lockout(false)
        , hit_current_not_reached(false)
    ...
    uint8_t getFaultCount() const {
        uint8_t count = 0;
        if (overcurrent_protection) count++;
        if (open_load) count++;
        if (plunger_movement) count++;
        if (undervoltage_lockout) count++;
        if (hit_current_not_reached) count++;
        if (thermal_shutdown) count++;
        return count;
    }
};
